...
function createReplyElement(reply, level = 0) {
    ...
    const userId = getLoggedInUserFromURL(); // example: ?user=pbugga14
    const voteStateKey = `voted-${userId}-${reply.id}`;
    const userVote = localStorage.getItem(voteStateKey);

    const voteStorageKey = `vote-count-${reply.id}`;
    let voteCount = parseInt(localStorage.getItem(voteStorageKey)) || reply.votes || 0;
    ...
    replyDiv.innerHTML = `
        ...
        <div class="vote-section">
            <button class="upvote-btn" ${userVote ? 'disabled' : ''}>↑</button>
            <button class="downvote-btn" ${userVote ? 'disabled' : ''}>↓</button>
    ...
    upvoteBtn.addEventListener("click", () => {
        if (!localStorage.getItem(voteStateKey)) {
            voteCount++;
            voteCountSpan.textContent = voteCount;
            localStorage.setItem(voteStorageKey, voteCount);
            localStorage.setItem(voteStateKey, "up");
    ...
    downvoteBtn.addEventListener("click", () => {
        if (!localStorage.getItem(voteStateKey)) {
            ...
            voteCountSpan.textContent = voteCount;
            localStorage.setItem(voteStorageKey, voteCount);
            localStorage.setItem(voteStateKey, "down");
            upvoteBtn.disabled = true;
            downvoteBtn.disabled = true;
        }
    });
